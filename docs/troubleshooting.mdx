...
If your admin account no longer has admin privileges, you can reset it by editing the owner account directly in the database.

### Solution: Reset admin access

1. Stop Seearr
2. Back up your `config/settings.json` file
3. Delete `config/settings.json`
4. Start Seearr again
5. This will force the setup page to appear
6. Go through the setup process with the same login details
7. Stop Seerr
8. Restore your backed-up `settings.json` file (except for the `owner` field)
9. Start Seearr again
