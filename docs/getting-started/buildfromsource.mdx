...
description: Install Seearr by building from source
sidebar_position: 2
---
...
## Unix (Linux, macOS)
### Installation
1. Assuming you want the working directory to be `/opt/seearr`, create the directory and navigate to it:
```bash
sudo mkdir -p /opt/seearr && cd /opt/seearr
```
2. Clone the Seearr repository and checkout the seearr branch:
```bash
...
:::info
You can now access Seearr by visiting `http://localhost:5055` in your web browser.
:::
