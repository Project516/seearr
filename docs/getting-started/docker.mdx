---
title: Docker
description: Install Seearr using Docker
sidebar_position: 1
---
# Docker
:::info
...
:::info
Our Docker images are available on GitHub Container Registry (GHCR) with the following tags:

- `seearr`: The latest stable release.
- `sha-<commit>`: Specific commit-based builds.

All images are multi-arch (amd64 and arm64).
:::
...
docker run -d \
  ...
  --health-interval 15s \
  --health-retries 3 \
  ghcr.io/project516/seearr:seearr
```
...
```bash
docker pull ghcr.io/project516/seearr:seearr
```
...
services:
  seerr:
    image: ghcr.io/project516/seearr:seearr
    init: true
